...
public class DrawDonkeyCart extends JFrame{
	static ResultSet rs;
	static String query;
	public static void showFram(String querynew)
	{
		query = querynew;
		DrawDonkeyCart f = new DrawDonkeyCart();
		f.setTitle("Donkey Cart");
		f.setBounds(20,20,400,400);
		f.setVisible(true);
	}
	public DrawDonkeyCart() {
	...
	static class myPanel extends JPanel {
		...
		public void paintComponent(Graphics g){
			...
			g.drawString("Donkey Cart", 20, 20);
			...
			try {
				...
				while (rs.next()) {
					...
					int size = ia4.length/2;
					int x[]= new int[size+1];
					int y[]= new int [size+1];

					//get type and interpretation
					int type = ia3[1];
					...
					//get the ordinate array and draw the respective figures
